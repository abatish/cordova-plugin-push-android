...
| -------------- | ----------- | --------------- | ----------- | --------- |
| 1.0.1          | 10.0.0      | 8.0.0           | 5.1.1       | 1.8.0     |

To install from the command line:

```bash
cordova plugin add cordova-plugin-push-android
```
...
```xml
<plugin name="cordova-plugin-push-android" spec="1.0.1" />
```
...
When some other packages include `cordova-google-play-services` as a dependency, such as is the case with the `cordova-admob` and `cordova-plugin-analytics` plugins, it is impossible to also add the `cordova-plugin-push-android`, for the following error will rise during the build process:
...
```log
Installing "cordova-plugin-push-android" for ios
Failed to install 'cordova-plugin-push-android':Error: pod: Command failed with exit code 1
